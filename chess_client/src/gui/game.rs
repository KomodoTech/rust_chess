...
pub async fn game_scene(color: PlayerColor, mut socket: QuadSocket) -> Scene {
    ...
    let mut board_array = init_board();
    let mut height;
    let mut width;
        ...
        mouse_state = match mouse_state {
            MouseState::Unclicked => {
                    ...
                    || board_y_margin + game_size <= mouse_y_pos
                {
                    ...
                    let piece = board_array[row][col].take();
            ...
            MouseState::Clicked { row, col, piece } => {
                    ...
                    || board_y_margin + game_size <= mouse_y_pos
                {
                    ...
                    board_array[new_row][new_col] = piece.or(board_array[new_row][new_col]);
                    MouseState::Unclicked
                }
            }
        };
        for (row_idx, row) in board_array.iter().enumerate() {
...
fn init_board() -> Board {
    let mut board_array: [[Option<Piece>; 8]; 8] = [[None; 8]; 8];
    for row in &mut board_array {
        row[0] = 'P'.try_into().ok();
        row[1] = 'p'.try_into().ok();
        row[2] = 'K'.try_into().ok();
        row[3] = 'k'.try_into().ok();
